...
void gcn_propagation(GCNNetwork *network) {
    ...
    while (p != NULL) {
        ...
        if (p->latent_vectors.col_size > p->hidden_layer.col_size) {
            ...
            #if defined(EMAX6) || defined(EMAX7)
                imax_dense_format_init_from_sparse(&imax_h, &network->graph->imax_matrix, p->latent_vectors.col_size, 8);
...
void gcn_backpropagation(GCNNetwork *network, DenseMatrix *labels, void (*optimizer)(DenseMatrix *value, DenseMatrix *grad, OptimizerOption *option), OptimizerOption *option) {
    ...
    #endif
    int layer_cnt = 1;
    while (p->next != NULL) {p = p->next;layer_cnt++;}
    ...
    while (p != NULL) {
        ...
        relu_time += cal_time(&t2, &t1);
        #ifdef USE_CUDA
            if (p->next == NULL) sendDenseMatrixToCPU(&w_error);
        #endif
        ...
        allocDenseMatrix(&r_spmm); allocDenseMatrix(&h_next_error);
        ...
        transpose(&w_transpose, &(p->hidden_layer));
        ...
        #if defined(EMAX6) || defined(EMAX7)
            imax_dense_format_init_from_sparse(&imax_h, &network->graph->imax_matrix, p->latent_vectors.col_size, 8);
            imax_dense_format_init_from_sparse(&imax_h_error, &network->graph->imax_matrix, h_error.col_size, 8);
            imax_dense_format_init_from_sparse(&imax_h_next_error, &network->graph->imax_matrix, p->latent_vectors.col_size, 8);
            imax_dense_format_init(&imax_h_transpose, imax_h.col_size, imax_h.row_size, imax_h.col_padded_size, imax_h.row_padded_size, imax_h.blk_row_size, imax_h.blk_col_size);
            imax_dense_format_init(&imax_r_spmm, imax_h.row_size, imax_h.col_size, imax_h.row_padded_size, imax_h.col_padded_size, imax_h.blk_row_size, imax_h.blk_col_size);
            imax_dense_format_init(&imax_w, imax_r_spmm.col_size, p->hidden_layer.col_size, imax_r_spmm.col_padded_size, p->hidden_layer.col_size + imax_r_spmm.blk_col_size - (p->hidden_layer.col_size%imax_r_spmm.blk_col_size), MM_H, imax_r_spmm.blk_col_size);
            ...
            convert_imax_dense_format(&imax_h_next_error, &h_next_error);
